...
parser.add_argument('-S', '--start', type=int, help='Used to specify a position in the reference before which we will ignore reads.')
parser.add_argument('-E', '--end', type=int, help='Used to specify a position in the reference after which we will ignore reads.')
parser.add_argument('--double-count-overlaps', action="store_true", help="""For paired reads, by default when we encounter the second in the pair we ignore mapped positions that were contained in the first (i.e. counting the overlap only once, which is what samtools mpileup does unless you specify --ignore-overlaps and/or --min-BQ 0). With this option, we will count the overlap twice.""")
args = parser.parse_args()

SeqList = list(SeqIO.parse(open(args.RefFile), 'fasta'))
if len(SeqList) != 1:
    print('There are', len(SeqList), 'sequences in', args.ref_file + '. There should be exactly 1. Quitting.', file=sys.stderr)
    sys.exit(1)
...
if not os.path.isfile(args.BamFile+'.bai'):
    try:
        ...
        print(args.BamFile, 'has no corresponding .bai file, and there was a problem calling samtools index to create this file. It is needed for pysam. Quitting.', file=sys.stderr)
        raise
...
if len(AllReferences) != 1:
    print('Expected exactly one reference in', args.BamFile+'; found', str(len(AllReferences))+'.Quitting.', file=sys.stderr)
    sys.exit(1)
RefName = AllReferences[0]

# Make the start and end positions zero-based if we have them. If we only have one (i.e. start or end), set the other manually, to facilitate the comparison of positions. Check end >= start.
start = args.start
end = args.end
HaveStart = start is not None
HaveEnd = end is not None
...
if HaveStart:
    ...
    if not HaveEnd:
        ...
        print('The end point should not be before the start point. Quitting.', file=sys.stderr)
        sys.exit(1)
if HaveEnd:
    end -= 1
    if not HaveStart:
...
# At each reference position record (1) the number of reads mapped here and (2) the sum of the identity values of reads mapped here. Each read contributes to a count of 1, and its identity value, to every position to which it is mapped.
CoveragesByPos = [0] * RefLength
IdentityTotalsByPos = [0] * RefLength
PositionsByRead = {}
for read in BamFile.fetch(RefName):
    ...
    # If this read is paired and we've seen its mate already, ignore mapped positions in this read that were contained in the mate. If we haven't seen the mate already, record this read's positions.
    if (not args.double_count_overlaps) and read.is_paired:
        if read.query_name in PositionsByRead:
            ...
            MappedPositions = [pos for pos in MappedPositions if pos not in MatePositions]
...
# Combine identity totals for positions that have the same coverage, and count how many positions have that coverage.
...
# The total number of reads present in all positions with a given coverage (counting each read once per position-that-has-that-coverage, not once in total) is the value of the coverage multiplied by the number of positions-that-have-that-coverage. Dividing the total read identity associated with this coverage (again, with each read contributing once per position-that-has-that-coverage) by that total number of reads gives the mean identity for that coverage.
