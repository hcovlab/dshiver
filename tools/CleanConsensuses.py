...
have_global_aln = args.global_aln is not None
have_individual_consensus = args.individual_consensus is not None
have_base_freqs = args.base_freqs is not None
num_input_types = sum([have_global_aln, have_base_freqs, have_individual_consensus])
if num_input_types != 1:
    print('Exactly one of the --global_aln, --individual_consensus,', '--base_freqs options should be specified. Quitting.', file=sys.stderr)
    exit(1)
...
# specified.
if have_individual_consensus and args.common_reference is None:
    print('The --individual_consensus option requires the --common_reference', 'option to be used too. Quitting.', file=sys.stderr)
    exit(1)

# Can't use some options with --base_freqs.
if have_base_freqs:
    if args.split_amplicons:
        print('The --split_amplicons option cannot be used with the --base_freqs', 'option. Quitting.', file=sys.stderr)
        exit(1)
    if args.print_new_seq_blacklist:
        print('The --print_new_seq_blacklist option cannot be used with the', '--base_freqs option. Quitting.', file=sys.stderr)
        exit(1)
    if args.keep_overhangs:
        print('The --keep_overhangs option cannot be used with the', '--base_freqs option. Quitting.', file=sys.stderr)
        exit(1)

# Read in the sanger to BEEHIVE ID dict if desired.
have_sanger_dict = args.sanger_to_beehive_id_dict is not None
if have_sanger_dict:
    ...
    with open(args.sanger_to_beehive_id_dict, 'r') as f:
        for line_num_min_1, line in enumerate(f):
            ...
            fields = line.split(",")
            if len(fields) != 2:
                print('Encountered', len(fields), 'fields in', args.sanger_to_beehive_id_dict + '. Expected 3. Quitting.', file=sys.stderr)
                exit(1)
            sanger_id, beehive_id = fields
            if sanger_id in sanger_to_beehive_id_dict:
                print('Encountered', sanger_id, 'a second time in', args.sanger_to_beehive_id_dict + '. Quitting.', file=sys.stderr)
                exit(1)
            if beehive_id in all_beehive_ids:
                print('Encountered', beehive_id, 'a second time in', args.sanger_to_beehive_id_dict + '. Quitting.', file=sys.stderr)
                exit(1)
...
with open(args.amplicon_regions_file, 'r') as f:
    for line in f:
        fields = line.split(',')
        if len(fields) != 3:
            print('Encountered', len(fields), 'fields in', args.amplicon_regions_file + '. Expected 3. Quitting.', file=sys.stderr)
            exit(1)
        region, start, end = fields
        start, end = int(start), int(end)
        if start > end:
            print('Start greater than end for region', region, 'in', args.amplicon_regions_file + '. Quitting.', file=sys.stderr)
            exit(1)
        if region in regions_dict:
            print('Encountered', region, 'a second time in', args.amplicon_regions_file + '. Quitting.', file=sys.stderr)
            exit(1)
        ...
        if regions_dict_not_empty:
            ...
            if start != previous_end + 1:
                print('Region', region, 'starts at', start, 'which is not 1 more than', 'the end of the previous region (' + str(previous_end), 'for', previous_region + '). Quitting.', file=sys.stderr)
                exit(1)
...
with open(args.seq_based_blacklist, 'r') as f:
  for line_num_min_1, line in enumerate(f):
    ...
    if len(fields) != num_fields:
      print('Unexpected number of fields', len(fields), 'on line', line_num_min_1 + 1, 'in', args.seq_based_blacklist + '; expected', str(num_fields) + '. Quitting.', file=sys.stderr)
      exit(1)

    # Coerce string bools to bools, and record them.
    values = fields[1:-1]
    if any(value != "TRUE" and value != "FALSE" for value in values):
      print('Unexpected value not equal to TRUE or FALSE on line', line_num_min_1 + 1, 'in', args.seq_based_blacklist + '. Quitting.', file=sys.stderr)
      exit(1)
    values = np.array([True if value == "TRUE" else False for value in values], dtype=bool)
...
if args.print_new_seq_blacklist is not None:
