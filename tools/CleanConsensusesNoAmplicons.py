...
have_dates = args.date_sampled_csv is not None
...
def preprocess_seq(seq):
  ...
  if any(base not in "ACGTN-" for base in seq_as_str):
    unexpected_bases = set(base for base in seq_as_str if base not in "ACGTN-")
...
def PropagateNoCoverageChar(seq, LeftToRightDone=False):
  ...
  assert "N-" not in ResultingSeq and "-N" not in ResultingSeq
  return ResultingSeq
...
with open(args.seq_based_blacklist, 'r') as f:
  seq_blacklist = {line.strip() for line in f}
...
for beehive_id, sub_seq_dict in seq_dict.items():
  ...
  if len(sub_seq_dict) == 1:
    ...
    assert seq_id_to_use not in final_seqs_dict
