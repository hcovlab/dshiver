...
if args.min_contig_size <= 0:
    print('The minimum contig size must be greater than 0. Quitting.', file=sys.stderr)
    exit(1)
...
if len(DuplicatedArgs) != 0:
    for DuplicatedArg in DuplicatedArgs:
        print('Contig name', DuplicatedArg, 'was duplicated in the arguments.', file=sys.stderr)
...
def split_parts(c, min_contig_size, split_gap_size, prefix=''):
    ...
    num_contig_bits_to_keep = sum(1 for bit in contig_bits if bit and len(bit) - bit.count('-') >= min_contig_size)
    ...
    for s in contig_bits:
        if s and len(s) - s.count('-') >= min_contig_size:
            ...
            if rename:
                ...
                label = prefix
            yield SeqRecord(seq=Seq('-' * pos + s + '-' * (aln_length - (pos + len(s)))), id=label, name=label, description='')
        pos = pos + len(s)
...
if MissingContigs:
    print("The following contigs were not found in " + args.alignment + ":", " ".join(MissingContigs) + '\nQuitting.', file=sys.stderr)
    exit(1)
...
for seq in AlignedSeqs:
    if seq.id in ContigsFound:
        if args.trim_overhangs:
            TrimmedSeq = "-" * FirstRefStart + str(seq.seq)[FirstRefStart:LastRefEnd + 1] + "-" * (AlignmentLength - LastRefEnd - 1)
            assert len(TrimmedSeq) == AlignmentLength, "Internal malfunction of overhang trimming"
            seq.seq = Seq(TrimmedSeq)
        SeqsForOutput += list(split_parts(seq, args.min_contig_size, args.split_gap_size))
    else:
...
if NumContigs == 0:
    print("After splitting contigs at gaps of length at least", args.split_gap_size, "and discarding contigs of length less than " + str(args.min_contig_size) + ", no contigs were left. Quitting.", file=sys.stderr)
    exit(3)
