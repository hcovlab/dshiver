    ...
    for i, pos in enumerate(positions):
        if pos is not None:
            if ReferenceSeq[pos] == seq[i]:
                NumAgreeingBases += 1
            if LastRefPos is not None and pos != LastRefPos + 1:
...
def TranslateSeqCoordsToAlnCoords(seq, coords):
    ...
    for GappyPostitionMin1, base in enumerate(seq):
        if base != '-':
            ...
            if -1 not in TranslatedCoords:
                break
    assert -1 not in TranslatedCoords
...
def RemoveBlankColumns(alignment, BlankChars="-", RemoveUninformative=False):
    ...
    optionally including other characters in the BlankChars arg), and optionally
    ...
    for column in reversed(range(AlignmentLength)):
        ...
        for base in alignment[:, column]:
            if base not in BlankChars:
                if RemoveUninformative:
                    if FirstBaseSeen is None:
                        FirstBaseSeen = base
