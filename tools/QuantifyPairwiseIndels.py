...
try:
    ...
    print('Problem reading', args.alignment.name + ':', file=sys.stderr)
...
# Figure out the alignment position -> reference position correspondence.
if args.reference:
    RefFound = False
    for seq in alignment:
        if seq.id == args.reference:
            if RefFound:
                print('At least two sequences in', args.alignment.name, 'are named',
            ...
            RefFound = True
    if not RefFound:
        print('Found no sequences in', args.alignment.name, 'named',
              args.reference + '. Quitting.', file=sys.stderr)
        sys.exit(1)
    if args.offset:
...
# False otherwise
seqs = np.array([np.array([base == '-' for base in seq.seq]) for seq in alignment])
