...
HavePairwiseAln = args.pairwise_aln is not None
if HavePairwiseAln and args.coords_file is not None:
    print('Specify either --coords-file or --pairwise-aln, not both. Quitting.', file=sys.stderr)
    sys.exit(1)
if not HavePairwiseAln and args.coords_file is None:
    print('Specify exactly one of --coords-file or --pairwise-aln. Quitting.', file=sys.stderr)
    sys.exit(1)
...
if len(BaseFreqData) < 2:
    print(args.coords_file, 'contains less than 2 lines; unexpected. Quitting.', file=sys.stderr)
    sys.exit(1)
...
if HavePairwiseAln:
    ...
    if not match:
        print('The first field of the first line of', args.coords_file, "should", "begin with 'position in ', followed by the reference name. It doesn't.", "Quitting.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print('Problem reading', args.pairwise_aln + ':', e, file=sys.stderr)
        sys.exit(1)
    if len(TwoSeqs) != 2:
        print('Found', len(TwoSeqs), 'sequences in', args.pairwise_aln + "; there should be exactly 2. Quitting.", file=sys.stderr)
        sys.exit(1)

    # Check one of the seqs has the right ref name; see which one.
    if TwoSeqs[0].id == RefName:
        if TwoSeqs[1].id == RefName:
            print('Both sequences in', args.pairwise_aln, 'are named', RefName, "(the reference name mentioned in the base freq file); only one should" "be. Quitting.", file=sys.stderr)
            sys.exit(1)
        RefSeq = TwoSeqs[0]
        OtherSeq = TwoSeqs[1]
    else:
        if TwoSeqs[1].id != RefName:
            print('Neither sequence in', args.pairwise_aln, 'is named', RefName, "(the reference name mentioned in the base freq file); exactly one " "should be. Quitting.", file=sys.stderr)
            sys.exit(1)
        RefSeq = TwoSeqs[1]
        OtherSeq = TwoSeqs[0]
    ...
    for AlnPosMin1 in range(len(TwoSeqs[0])):
    ...
    with open(args.coords_file, 'r') as f:
        ...
        for LineNumberMin1, fields in enumerate(reader):
            ...
            try:
                ...
                print('Not enough fields on line', LineNumberMin1+1, 'in', args.coords_file +'. Quitting.', file=sys.stderr)
                sys.exit(1)
            if RefPos != '-':
                try:
                    RefPos = int(RefPos)
                except IndexError:
                    print('Failed to understand the reference position as an' ' integer on line', LineNumberMin1+1, 'in', args.coords_file +'. Quitting.', file=sys.stderr)
                    sys.exit(1)
...
for LineNumberMin2, line in enumerate(BaseFreqData[1:]):
    ...
    try:
        ...
        print('Not enough fields on line', LineNumberMin2+2, 'in', args.BaseFreqFile +'. Quitting.', file=sys.stderr)
        sys.exit(1)
    try:
    ...
    if RefPos != LastRefPos + 1:
        print('Error: the reference position jumped from', LastRefPos, 'to', RefPos, 'on line', LineNumberMin2+2, 'in', args.BaseFreqFile + '. Quitting.', file=sys.stderr)
        sys.exit(1)
    LastRefPos = RefPos
    ...
    if HavePairwiseAln and RefBase.upper() != GaplessRefSeq[RefPos-1]:
        print('Error: mismatch in reference base at position', str(RefPos) + ':', RefBase, 'in', args.BaseFreqFile, 'but', GaplessRefSeq[RefPos-1], 'in', args.pairwise_aln + '. Quitting.', file=sys.stderr)
        sys.exit(1)
