...
for pos in range(AlignmentLength):
    ...
    for base in BasesObserved:
        if base not in OKbases:
            del BaseCounts[base]
    if len(BaseCounts) == 0:
        print('No "OK" bases, i.e.', OKbases + ", were observed at position", pos + 1,
              "in", args.alignment + ". This code does not know how to estimate",
...
for row in range(len(MutableSeqList)):
    ...
    for pos in range(AlignmentLength):
        ...
        if base not in OKbases:
            ...
            # Convert to upper case, but remember whether it was originally lower
            IsLower = base == base.lower()
            UpperBase = base.upper()

            # Which bases does this ambiguity code mean?
            if UpperBase not in IUPACdict:
                print("Error: unexpected base", base, 'at position', pos + 1,
                      'for seq', ID, 'in', args.alignment + ". Quitting.", file=sys.stderr)
                exit(1)
